...
class Analyzer(Agent):
    ...
    TEMPLATE = (
        ...
        "didática adulta, storytelling funcional, metáforas inteligentes, WIIFM executivo, "
        "ROI, governança e trade-offs para CEO/CIO/Conselho\n"
        "3. FORMATAÇÃO: tabelas por módulo, hierarquia de títulos, negrito, blocos de citação, "
        "parágrafos justificados (text-justify ou <p align=\"justify\"> em HTML exportado)\n"
    ...
    def build_prompt(self, context: str, **template_vars: str) -> str:
        """Build the analysis prompt, filling all named placeholders.

        analyze.md uses {course_name} and {draft_content} — not {context}.
