...
def main() -> None:
    """Corpo do script; só roda quando chamado diretamente, nunca no import."""
    ...
    check(
        "draft real encontrado", bool(real_drafts), str(real_drafts[0].name) if real_drafts else ""
    )
    ...
    print(
        f"  Voice Guard OK:       {result.voice_guard_ok}  (score {result.voice_guard_score}/100)"
    )
    ...
    check(
        "sem vazamento ACME", "ACME Consultoria" not in page_tsx and "Maria Silva" not in page_tsx
    )
