...
- [`docs/research/geo-wave-junho-15-2026/GEO_WAVE_JUNHO_15_2026_CANONICAL.md`](docs/research/geo-wave-junho-15-2026/GEO_WAVE_JUNHO_15_2026_CANONICAL.md) — **doc canônico**, delta 07-jun → 15-jun, ancorado em pesquisa web verificada ao vivo (WebSearch/WebFetch jun/2026) + orchestrator 5 LLMs no fundamento técnico atemporal (§5). Vira material didático direto: (a) **módulo novo "Share of Voice vs Share of Answer"** (estar na lista de fontes ≠ ser a resposta) + bateria de KPIs (Answer Inclusion Rate, Recommendation Rate, Prompt Coverage, Assisted Click Yield) + report mensal de 8 seções; (b) **módulo novo "Relevance Engineering"** (Michael King/iPullRank — recuperação de informação + IA + conteúdo); (c) **rubrica de redação** (`GEO_REDACAO_CHECKLIST_2026.md`) ganha os 3 níveis do GEO-SFE `2603.29979` ✓ (macro = arquitetura do documento, meso = chunking/answer capsules, micro = ênfase de termos/números; +17,3% citation rate) carimbados em `draft.md` e validados no `content_checker.py`; o `reviewer.py` adota o checklist diagnóstico-reparo do AgentGEO `2603.09296` ✓ (+40% citação mudando 5%); (d) tags `classify.md` ganham `share-of-answer`, `relevance-engineering`, `structural-features`, `agentic-commerce`; (e) **módulo novo "Comércio agêntico"** (ads no ChatGPT 09-fev, guerra de agentes de compra, AIPREF vs llms.txt). Benchmark de aula: RCT de AIO (-38% cliques) e Seer (citado = +120% cliques/impressão). **§7.3 é a aplicação específica deste repo** (3 módulos novos + GEO-SFE na rubrica + AgentGEO no reviewer + tags). Raw do orchestrator em [`raw/`](docs/research/geo-wave-junho-15-2026/raw/).
