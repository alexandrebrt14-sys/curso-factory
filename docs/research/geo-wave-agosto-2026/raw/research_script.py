...
SYSTEM = (
    "Você é pesquisador sênior de GEO (Generative Engine Optimization) e SEO. Responda em português do Brasil "
...
async def pplx(c, name, prompt, model, max_tokens):
    try:
        b = {
            ...
            "search_recency_filter": "month" if model == "sonar-pro" else "year",
        }
        r = await c.post(
            "https://api.perplexity.ai/chat/completions",
            headers={"Authorization": f"Bearer {K('PERPLEXITY_API_KEY')}"},
            json=b,
        )
        r.raise_for_status()
        d = r.json()
        ...
        if not text.strip():
            raise RuntimeError(f"resposta vazia: {json.dumps(d)[:300]}")
        cits = [
            x if isinstance(x, str) else (x.get("url") or str(x))
...
async def gem(c, name, prompt):
    try:
        ...
        r = await c.post(
            ...
            headers={"x-goog-api-key": K("GOOGLE_AI_API_KEY")},
            json=b,
        )
        r.raise_for_status()
        d = r.json()
        ...
        srcs = [
            f"{(ch.get('web') or {}).get('title', '')} — {(ch.get('web') or {}).get('uri')}"
            ...
            for ct in item.get("content", []) or []:
                if ct.get("type") == "output_text":
                    ...
                    for a in ct.get("annotations", []) or []:
                        if a.get("url"):
                            srcs.append(f"{a.get('title', '')} — {a['url']}")
        if not text.strip():
