...
- **Densidade visual:** o padrão é o curso `frontends-com-vibecoding` (94 figuras SVG, 113 callouts). Não entregue paredão de texto. Figura = bloco `{type:"figure", value:<svg inline>, label}`, `role="img"` + `<title>`, cores por token, e texto sobre acento com `var(--course-accent-fg,#fff)` em vez de `#ffffff` cravado (branco fixo reprova AA em curso de acento claro). Ritmo: nunca mais que 2 blocos `text` seguidos sem um visual. Os três limites verificáveis, o vocabulário de peças e as armadilhas de payload (bloco sem `data` some calado, `comparison` tem valência fixa, `flow` tem duas formas incompatíveis) estão em `docs/DOUTRINA_VISUAL_CURSOS.md`.
