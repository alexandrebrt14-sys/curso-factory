...
- **Peso visual da saída:** o gerador emite seis blocos visuais além dos de prosa: `figure`, `dataTable`, `comparison`, `statGrid`, `stepGuide` e `timeline`. São exatamente os que o portão `scripts/gate-peso-visual.mjs` da landing conta, e as cargas seguem as do motor de cursos. O vocabulário vive em quatro lugares que mudam **sempre juntos** (`src/models.py::SectionType`, `src/schemas/course.schema.json`, `src/templates/page.tsx.j2` e o filtro `js_json` de `src/generators/tsx_generator.py`); mexer em um só produz bloco que o validador aceita e a página não desenha. Tabela, lista numerada de passos e imagem com legenda são promovidas automaticamente pelo parser; `comparison`, `statGrid` e `timeline` entram por autoria explícita. Limites, catálogo de peças e armadilhas em `docs/DOUTRINA_VISUAL_CURSOS.md`.
