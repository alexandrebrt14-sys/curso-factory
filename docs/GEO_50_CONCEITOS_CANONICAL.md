...
## Aplicação em curso-factory

Estes 50 conceitos são a SPINE didática de todo curso de GEO/SEO produzido na fábrica. Cada conceito = potencialmente 1 aula HBR-grade. Verticais (joalheria, fintech, jurídico, saúde) reaproveitam a mesma taxonomia, adaptando exemplos.

Operacionalização no pipeline da fábrica:

- **Planejamento de trilha** — toda nova trilha de GEO/SEO em `config/courses.yaml` ou em uma nova vertical em `config/clients/<id>/` deve mapear quais dos 50 conceitos compõem a espinha. Recomendação: 1 conceito = 1 aula, com módulos agrupando conceitos do mesmo eixo (1-3 técnicos, 4-7 on-page, 8-12 conteúdo, etc.).
- **Agente de pesquisa (Perplexity)** — quando o tema da aula casa com um dos 50, ancorar a investigação no nome canônico do conceito e nas referências da seção final deste documento.
- **Agente de redação (GPT-4o)** — abrir a aula com a definição literal do conceito ("Mede se..."), depois expandir em mecanismo, evidência e aplicação vertical (joalheria, fintech, jurídico, saúde, educação).
- **Agente de análise (Gemini)** — verificar se a aula gerada respeita o eixo declarado e se exemplos vão além do nicho default.
- **Agente de revisão (Claude)** — checar se nenhum anti-padrão da seção final aparece como afirmação positiva (pseudo-GEO, schema inflado, llms.txt como talismã, slugs com acento, pseudo-autoridade).
- **Anti-padrões (Conceito 26)** — toda menção a pseudo-GEO vira aula de "anti-padrão a evitar", nunca aula de "como conseguir citação garantida".
